...
То есть VACUUM(FULL) - переносит таблицу без пропусков в новый файл, тем самым уменьшая ее размер, когад как обычный VACUUM позволяет переиспользовать пропуски.
